...
| `proef flaky` | flakiness verdicts over the retained run history — flapping, passes-only-on-retry, always-failing, plus quarantine's own two failure modes; keyed by the run's input fingerprint so a pack/config edit ends the window; `--by <key>` splits per environment or `[meta]` value; `--min-samples`/`--recovery-runs`/`--outage-rate` tune the statistical guards (`--format json`) |
