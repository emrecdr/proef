...
| `proef test [path]` | validate + execute (`--env`, `--dry-run`, `--tags`, `--scenario`, `--scenario-file`, `--jobs`, `--junit`, `--run-id`, `--rerun`, `--sarif`, `--format json|tap`, `--watch`, `--shard I/N`, `--max-fail N`, `--shuffle`, `--meta KEY=VALUE`, `--console full|failed|dotted|quiet`); path optional — defaults to `[run] suite`, then `tests/` |
