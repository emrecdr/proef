...
| 6 | Self-contained HTML report | **shipped** | ✅ | core `render_html(&[Event])` + cli write | M | Post-hoc `proef report --html <run-id>` replaying `events.jsonl` like `explain` (`explain.rs:12`). Bodies live in `artifacts/` — deep-link, don't inline. Derived view, never a second record (ADR-0008). |
