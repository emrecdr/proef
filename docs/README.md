...
| [0021](adr/ADR-0021-run-discovery-and-rotation-are-separate-questions.md) | Run discovery and rotation are separate questions: a directory is a record because it holds one, rotation still deletes only uuid-named dirs, ordering follows the record's own start time | Accepted |
