...
///
/// Keep a Changelog's kinds (`Added`/`Changed`/`Fixed`/…) are the reader's index
/// into a release: "what broke for me" is one heading, and a release carrying
/// four separate `### Fixed` blocks has no such index — the reader must read all
/// four to know they have read them all. Thirty-four such repeats had
/// accumulated by 0.16, appended a wave at a time because appending is what
/// writing a changelog feels like.
///
/// Written as a test rather than left as a tidy file, because the file was tidy
/// once: the repeats are what *appending* produces, and nothing else here would
/// notice the thirty-fifth. The same reasoning as every other guard in this
/// suite — a property that lives only in a past commit is a property that decays.
///
/// The kind is the text **before any parenthetical**: two 0.2-era releases spell
/// theirs `### Fixed (v0.1.0 deep-review follow-up …)`, and comparing raw
/// heading text would let a plain `### Fixed` sit beside one undetected. That
/// is precisely the collision worth catching, so the comparison normalizes the
/// way a reader does.
#[test]
fn a_release_names_each_kind_of_change_once() {
    let text = std::fs::read_to_string(workspace_root().join("docs/CHANGELOG.md")).unwrap();
    ...
    for (no, line) in text.lines().enumerate() {
        if let Some(heading) = line.strip_prefix("## ") {
            ...
            let kind = heading.split('(').next().unwrap_or(heading).trim();
            if let Some((_, first)) = seen.iter().find(|(name, _)| *name == kind) {
                repeats.push(format!(
                    "`{release}` names `{kind}` twice — lines {first} and {}",
                    no + 1
                ));
            } else {
                seen.push((kind, no + 1));
    ...
    // Vacuity guard: a parser that silently stopped matching would report a
    // clean changelog, which is the one result this test must never invent.
    assert!(releases >= 15, "parsed only {releases} release sections");
    assert!(
        repeats.is_empty(),
        "each release must name a kind of change once:\n  {}",
        repeats.join("\n  ")
    );
}
